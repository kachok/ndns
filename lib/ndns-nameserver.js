//
// MODULE TO IMPLEMENT DNS SERVER FOR THE NDNS LOAD BALANCER
//
...
exports.addToTree = function(tree, array , value) {
...
function checkQuestions(q, rr, p_type_syms) {
  var result = false;
...
exports.createResponse = function(req, res, root, p_type_syms) {
  ...
  for( var i = 0; i < 3; i++) {
    ...
    for( var j = 0; j < req.q.length; j++){
      ...
      for( var k = 0; k < root.length; k++) {
        ...
        if(checkQuestions(req.q[j], root[k], p_type_syms) && i == 0) {
          addRR(res,root[k]);
          count[i]++;
        }  
